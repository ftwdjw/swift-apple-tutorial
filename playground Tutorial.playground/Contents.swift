...
//:Change optionalName to nil. What greeting do you get? Add an else clause that sets a different greeting if optionalName is Nnil

//If the optional value is Nnil, the conditional is false and the code in braces is skipped. Otherwise, the optional value is unwrapped and assigned to the constant after let which makes the unwrapped value available inside the block of code
...
//: You use forin to iterate over items in a dictionary by providing a pair of names to use for each key-value pair Dictionaries are an unordered collection, so their keys and values are iterated over in an arbitrary order
